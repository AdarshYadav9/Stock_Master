...
const Adjustments = () => {
  ...
  const { adjustments, loading, fetchAdjustments } = useOperationsStore();
  ...
  const filteredAdjustments = adjustments.filter((adjustment) => {
    return statusFilter === 'all' || adjustment.status === statusFilter;
  });
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-foreground">Inventory Adjustments</h1>
            <p className="text-muted-foreground mt-1">Manage stock adjustments and corrections</p>
          </div>
                ...
                <TableBody>
                  {filteredAdjustments.map((adjustment) => (
                    <TableRow key={adjustment.id}>
                      <TableCell className="font-mono font-medium">{adjustment.reference}</TableCell>
                      <TableCell>
                        {warehouses.find((w) => w.id === adjustment.warehouse)?.name ||
                          adjustment.warehouse}
                      </TableCell>
                      <TableCell>{adjustment.reason}</TableCell>
                      <TableCell>{adjustment.items.length} items</TableCell>
                      <TableCell>
